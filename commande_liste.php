...
// SUPPRESSION
if (isset($_GET['ids'])) {
    $bd->query('delete from commande where idCommande=' . $_GET['ids']);
    // header('Location: commande_cours.php');
}
elseif (isset($_GET['idTerminer'])) {
    $bd->query('UPDATE commande SET etatCommande="Termine" where idCommande=' . $_GET['idTerminer']);
    // header('Location: commande_cours.php');
}
elseif (isset($_GET['idLivrer'])) {
    $bd->query('UPDATE commande SET etatCommande="Livré" where idCommande=' . $_GET['idLivrer']);
    // header('Location: commande_cours.php');
}
?>
<!DOCTYPE html>
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-7 align-self-center">
                        <h4 class="page-title text-truncate text-dark font-weight-medium mb-1">Liste Commandes</h4>
                        <div class="d-flex align-items-center">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb m-0 p-0">
                                    <li class="breadcrumb-item"><a href="#" class="text-muted">Vues</a></li>
                                    <li class="breadcrumb-item text-muted active" aria-current="page">Liste Commandes</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title text-center">COMMANDES</h4>
                                <div class="table-responsive">
                                    <table id="default_order"
                                        ...
                                        <tbody>
                                            <?php                                            
                                                $req = $bd->query ('SELECT * FROM commande,client,mesure WHERE commande.idClient=client.idClient and commande.idMesure=mesure.idMesure');
                                                $i = 1;
                                               // var_dump($req->fetch());
                                                
                                                while ($ligne = $req->fetch()) {
                                                    echo '<tr>';
                                                    echo '<td>' . $ligne['nomC'] .' '.$ligne['prenomC'].'</td>';
                                                    echo '<td>' . $ligne['typeVetement'] .'</td>';
                                                    // echo '<td>' . $ligne['<img src="Images/.$ligne[image]">'].'</td>';
                                                    echo '<td>' . $ligne['epaule'].
                                                ''.$ligne['manche'].
                                                ''.$ligne['poitrine'].
                                                ''.$ligne['taille'].
                                                ''.$ligne['cuisse'].
                                                ...
                                                echo '<td>'. $ligne['avance'].'</td>';
                                                echo '<td>'.  $ligne['reste'].'</td>';
                                                echo '<td>'. $ligne['dateDepot'].'</td>';
                                                echo '<td>'.$ligne['dateLivraison'].'</td>';
                                                echo '<td>'.$ligne['etatCommande'].'</td>';
                                                echo '<td>
                                                <button type="button"
                                                        class="btn waves-effect waves-light btn-rounded btn-outline-secondary">
                                                        <a href="commande.php?idm=' . $ligne['idCommande'] . '">Editer</a>
                                                    </button>
                                                    <button type="button"
                                                        class="btn waves-effect waves-light btn-rounded btn-outline-info">
                                                        <a href="commande_liste.php?idTerminer=' . $ligne['idCommande'] . '">Terminer</a>
                                                    </button>
                                                    <button type="button"
                                                        class="btn waves-effect waves-light btn-rounded btn-outline-success">
                                                        <a href="commande_liste.php?idLivrer=' . $ligne['idCommande'] . '">Livrer</a>
                                                    </button>
                                                    <button type="button"
                                                        class="btn waves-effect waves-light btn-rounded btn-outline-danger">
                                                        <a href="commande_liste.php?ids=' . $ligne['idCommande'] . '">Supprimer</a>
                                                    </button>
                                                </td>';
