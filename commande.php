...
if (isset($_POST['enregistrer'])) {
    if (isset($_POST['idClient'],$_POST['typeVetement'], $_POST['idMode'], $_POST['idMesure'],$_POST['prix'],$_POST['avance'],$_POST['reste'], $_POST['dateDepot'], $_POST['dateLivraison']));
    $req = $bd->prepare('INSERT into commande (idClient,typeVetement,idMode,idMesure,prix,avance,reste,dateDepot,dateLivraison) values(?,?,?,?,?,?,?,?,?)');
    $req->bindValue(1, $_POST['idClient']);
    $req->bindValue(2, $_POST['typeVetement']);
    $req->bindValue(3, $_POST['idMode']);
    $req->bindValue(4, $_POST['idMesure']);
    $req->bindValue(5, $_POST['prix']);
    $req->bindValue(6, $_POST['avance']);
    $req->bindValue(7, $_POST['reste']);
    $req->bindValue(8, $_POST['dateDepot']);
    $req->bindValue(9, $_POST['dateLivraison']);
    $req->execute();
    header('Location: commande_liste.php');
}

// EDITION
if (isset($_GET['idm'])) {
    $req = $bd->prepare('SELECT * from commande where idCommande=' . $_GET['idm']);
    if ($ligne = $req->fetch()) {
        $_POST['idCommande'] = $ligne['idCommande'];
        $_POST['idClient'] = $ligne['idClient'];
        $_POST['typeVetement'] = $ligne['typeVetement'];
        $_POST['idMode'] = $ligne['idMode'];
        $_POST['idMesure'] = $ligne['idMesure'];
        $_POST['prix'] = $ligne['prix'];
        $_POST['avance'] = $ligne['avance'];
        $_POST['reste'] = $ligne['reste'];
        $_POST['dateDepot'] = $ligne['dateDepot'];
        $_POST['dateLivraison'] = $ligne['dateLivraison'];
        $_POST['etatCommande'] = $ligne['etatCommande'];
    }
}
//Modification
if (isset($_POST['modifier'])) {
    if (isset($_POST['idClient'],$_POST['typeVetement'], $_POST['idMode'], $_POST['idMesure'], $_POST['prix'],$_POST['avance'],$_POST['reste'],$_POST['dateDepot'], $_POST['dateLivraison'], $_POST['etatCommande']));
    $req = $bd->prepare('UPDATE commande set idClient=? typeVetement=?,idMode=?,idMesure=?,prix=?,avance=?,reste=?,dateDepot=?,dateLivraison=?, etatCommande=? where idCommande=?');
    $req->bindValue(1, $_POST['idClient']);
    $req->bindValue(2, $_POST['typeVetement']);
    $req->bindValue(3, $_POST['idMode']);
    $req->bindValue(4, $_POST['idMesure']);
    ...
    $req->bindValue(6, $_POST['avance']);
    $req->bindValue(7, $_POST['reste']);
    $req->bindValue(9, $_POST['dateDepot']);
    $req->bindValue(10, $_POST['dateLivraison']);
    $req->bindValue(11, $_POST['etatCommande']);
    $req->bindValue(12, $_POST['idCommande']);
    $req->execute();
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-7 align-self-center">
                        <h4 class="page-title text-truncate text-dark font-weight-medium mb-1">Commande</h4>
                        <div class="d-flex align-items-center">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb m-0 p-0">
                                    <li class="breadcrumb-item"><a href="index.html" class="text-muted">Formulaires</a>
                                    </li>
                                    <li class="breadcrumb-item text-muted active" aria-current="page">Commande</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title text-center">Formulaire Commande</h4>
                                <form class="mt-4" action="commande.php" method="post">
                                    <div class="form-group">
                                        <select name="idClient" class="form-select" id="inputGroupSelect01">
                                            <?php
                                                $req = $bd->query ('SELECT * FROM client');

                                                while ($ligne=$req->fetch()) {
                                                    if (isset($_POST['idClient'])&& $ligne['idClient']==$_POST['idClient']) {
                                                        echo '<option value="'.$ligne['idClient'].'" selected>'
                                                        .$ligne['nomC'].''.$ligne['prenomC'].'</option>';
                                                    }
                                                else {
                                                    echo '<option value="'.$ligne['idClient'].'">'
                                                    .$ligne['nomC'].''.$ligne['prenomC'].'</option>';
                                                }
                                                }
                                            ?>
                                    ...
                                    <div class=" form-group" style="margin-top: 1rem;">
                                        <select name="typeVetement" class="form-select" id="inputGroupSelect01">
                                            <option>
                                                <?php if (isset($_POST['typeVetement'])) echo $_POST['typeVetement'] ?>
                                            </option>
                                            ...
                                            <option value="Bokolan">Bokolan</option>
                                        </select>
                                    </div>
                                    <div class="form-group" style="margin-top: 1rem;">
                                        <select name="idMode" class="form-select" id="inputGroupSelect01">
                                            <?php
                                                $req = $bd->query ('SELECT * FROM modele');

                                                    while ($ligne = $req->fetch()) {
                                                        if (isset($_POST['idMode'])&& $ligne['idMode']==$_POST['idMode']) {
                                                            echo '<option value="'.$ligne['idMode'].'" selected>'
                                                            .$ligne['<img src="Images/.$ligne[image]">'].'</option>';
                                                        }
                                                    else {
                                                        echo '<option value="'.$ligne['idMode'].'">'
                                                        .$ligne['<img src="Images/.$ligne[image]">'].'</option>';
                                                    }
                                                    }
                                            ?>
                                    ...
                                    <div class="form-group" style="margin-top: 1rem;">
                                        <select name="idMesure" class="form-select" id="inputGroupSelect01">
                                            <?php
                                                $req = $bd->query ('SELECT * FROM mesure');

                                                while ($ligne = $req->fetch()) {
                                                    if (isset($_POST['idMesure'])&& $ligne['idMesure']==$_POST['idMesure']) {
                                                        echo '<option value="'.$ligne['idMesure'].'" selected>
                                                        '.$ligne['epaule'].'
                                                    '.$ligne['manche'].'
                                                    '.$ligne['poitrine'].'
                                                        '.$ligne['taille'].'
                                                    ...
                                                    echo '<option value="'.$ligne['idMesure'].'">               
                                                    '.$ligne['epaule'].'
                                                    '.$ligne['manche'].'
                                                    '.$ligne['poitrine'].'
                                                    '.$ligne['taille'].'
                                    ...
                                    <div class="form-group" style="margin-top: 1rem;">
                                        <input name="prix" type="text" class="form-control" id="placeholder"
                                            placeholder="prix"
                                            value="<?php if (isset($_POST['prix'])) echo $_POST['prix'] ?>">
                                    </div>
                                    <div class="form-group" style="margin-top: 1rem;">
                                        <input name="avance" type="text" class="form-control" id="placeholder"
                                            placeholder="Avance"
                                            value="<?php if (isset($_POST['avance'])) echo $_POST['avance'] ?>">
                                    </div>
                                    <div class="form-group" style="margin-top: 1rem;">
                                        <input name="reste" type="text" class="form-control" id="placeholder"
                                            placeholder="Reste"
                                            value="<?php if (isset($_POST['reste'])) echo $_POST['reste'] ?>">
                                    </div>
                                    <div class="form-group" style="margin-top: 1rem;">
                                        <input name="dateDepot" type="date" class="form-control" id="placeholder"
                                            placeholder="Date Depot"
                                            value="<?php if (isset($_POST['dateDepot'])) echo $_POST['dateDepot'] ?>">
                                    </div>
                                    <div class="form-group" style="margin-top: 1rem;">
                                        <input name="dateLivraison" type="date" class="form-control" id="placeholder"
                                            placeholder="Date Livraison"
                                            value="<?php if (isset($_POST['dateLivraison'])) echo $_POST['dateLivraison'] ?>">
                                    </div>
                                    <?php { ?>
                                    <div class="form-group" style="margin-top: 1rem;">
                                        <input type="submit"
