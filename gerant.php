...
if (isset($_POST['enregistrer'])) {
    if (isset($_POST['nom'], $_POST['prenom'], $_POST['telephone'], $_POST['adresse']))
        ;
    $req = $bd->prepare('INSERT into gerant values(nom,prenom,telephone,adresse) ');
    $req->bindValue(1, $_POST['nom']);
    $req->bindValue(2, $_POST['prenom']);
    $req->bindValue(3, $_POST['telephone']);
    $req->bindValue(4, $_POST['adresse']);
    $req->execute();
    header('Location:gerant_liste.php');
}
// EDITION
if (isset($_GET['idm'])) {
    $req = $bd->query('SELECT * from gerant where idGerant=' . $_GET['idm']);
    if ($ligne = $req->fetch()) {
        $_POST['idGerant'] = $ligne['idGerant'];
        $_POST['nom'] = $ligne['nom'];
...
if (isset($_POST['modification'])) {
    if (isset($_POST['nom'], $_POST['prenom'], $_POST['telephone'], $_POST['adresse'], $_POST['idGerant']))
        ;
    $req = $bd->prepare('UPDATE gerant set  nom=?, prenom=?, telephone=?, adresse=? where idGerant=?');
    $req->bindValue(1, $_POST['nom']);
    $req->bindValue(2, $_POST['prenom']);
    $req->bindValue(3, $_POST['telephone']);
    $req->bindValue(4, $_POST['adresse']);
    $req->bindValue(5, $_POST['idGerant']);
    $req->execute();
    header('Location:gerant_liste.php');
}
?>
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-7 align-self-center">
                        <h4 class="page-title text-truncate text-dark font-weight-medium mb-1">Gerant</h4>
                        <div class="d-flex align-items-center">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb m-0 p-0">
                                    <li class="breadcrumb-item"><a href="index.html" class="text-muted">Formulaires</a>
                                    </li>
                                    <li class="breadcrumb-item text-muted active" aria-current="page">Gerant</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title text-center">Formulaire Gerant</h4>
                                <form class="mt-4" action="gerant.php" method="post">
                                    <div class="form-group">
                                        <?php
                                        if (isset($_GET['idm'])) {
                                            ?>
                                            <input type="hidden" name="idGerant" value="<?= $_GET['idm'] ?>">
                                            <?php
                                        }
                                        ?>
                                    ...
                                    <?php { ?>
                                        <div class="form-group" style="margin-top: 1rem;">
                                            <input type="submit"
                                                ...
                                                value="Enregistrer"
                                                name="<?= isset($_GET['idm']) ? "modification" : "enregistrer" ?>">
                                        </div>
                                    <?php } ?>
                                </form>
                            </div>
